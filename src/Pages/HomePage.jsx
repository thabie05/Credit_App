...
export default function HomePage() {
  ...
  const openProfileModal = (groupName) => {
    ...
    if (profile) {
      ...
      setProfileForm({ name: profile.name, phone: profile.phone || '', email: profile.email || '', address: profile.address || '', notes: profile.notes || '' });
    } else {
      setSelectedProfile(null);
      setProfileForm({ name: groupName, phone: '', email: '', address: '', notes: '' });
    }
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      if (selectedProfile) { await updateProfile(selectedProfile.id, profileForm); addToast('✅ Profile updated.'); }
      else { await addProfile(profileForm); addToast('✅ Profile created.'); }
      setShowProfileModal(false);
    } catch { addToast('❌ Failed to save profile.'); }
  };

  const handleDeleteDebt = async (firestoreId, name) => {
    if (!window.confirm(`Delete this debt for "${name}"?`)) return;
    try { await deleteDebt(firestoreId); addToast('🗑 Debt deleted.'); } catch { addToast('❌ Failed.'); }
  };
  ...
  const handleDeleteRepayment = async (debtId, repayIndex, name) => {
    ...
    if (debt.dueDate) {
      const dueDate = new Date(debt.dueDate + 'T00:00:00');
      const today = new Date(getTodayDate() + 'T00:00:00');
      if (today > dueDate) totalOwedWithPenalty += fullReturn * (debt.penaltyRate / 100) * monthsBetween(debt.dueDate, getTodayDate());
    }
    ...
    try {
      ...
      await updateDoc(doc(db, 'debts', debtId), { repayments: updatedRepayments, returnAmountOwed: newReturnOwed, paid: isPaid, paidDate: isPaid ? null : debt.paidDate });
      addToast('🗑 Repayment deleted.');
  ...
  const submitQuickAdd = async (groupKey, displayName) => {
    ...
    try {
      await addDebt({ name: displayName, borrowedAmount: amount, note: input.note || '', interestRate: input.interest || interestRate, date: input.date || getTodayDate(), isCompound: input.isCompound || false, dueDate: input.dueDate || null, penaltyRate: input.penalty || 50 });
      addToast(`✅ Added R${amount.toFixed(2)} to "${displayName}".`);
      setQuickAddInputs(prev => ({ ...prev, [groupKey]: { active: false, amount: '', interest: interestRate, date: getTodayDate(), note: '', isCompound: false, dueDate: '', penalty: 50 } }));
    } catch { addToast('❌ Failed.'); }
  ...
  const handleAddDebt = async () => {
    ...
    try {
      ...
      addToast(`✅ "${name}" added.`); setNewName(''); setNewAmount(''); setNewNote('');
      setDebtInterestRate(interestRate); setNewDate(getTodayDate()); setIsCompound(false); setDueDate(''); setPenaltyRate(50); setShowDebtForm(false);
    } catch { addToast('❌ Failed.'); }
  ...
  const submitRepayment = async (groupKey) => {
    ...
    try {
      const repaymentDate = input.date || getTodayDate();
      const debtIds = group.individualDebts.filter(d => !d.paid && d.remaining > 0).map(d => d.firestoreId);
      await addGroupRepayment(debtIds, amount, repaymentDate);
      addToast(`💵 Repaid ${formatCurrency(amount)} on ${formatDate(repaymentDate)}.`);
      setRepayInputs(prev => ({ ...prev, [groupKey]: { active: false, amount: '', date: getTodayDate() } }));
    } catch { addToast('❌ Failed.'); }
  };

  const handleDeletePerson = async (group) => {
    if (!window.confirm(`Delete ALL debts for "${group.displayName}"?`)) return;
    try { for (const id of group.firestoreIds) await deleteDebt(id); addToast(`🗑 Deleted.`); } catch { addToast('❌ Failed.'); }
  };

  const handleClearPaid = async () => {
    const paid = groupedDebtors.filter(g => g.isFullyPaid);
    if (paid.length === 0) { addToast('ℹ️ No paid debtors.'); return; }
    if (!window.confirm(`Remove all ${paid.length} paid?`)) return;
    try { for (const g of paid) for (const id of g.firestoreIds) await deleteDebt(id); addToast(`🗑 Cleared.`); } catch { addToast('❌ Failed.'); }
  };
  ...
  return (
    <div className="space-y-6">
                  ...
                  {hasHistory && (
                    <div className="mb-3">
                      <button onClick={() => toggleHistory(groupKey)} className="w-full py-2 bg-gray-950/50 hover:bg-gray-950/70 border border-white/5 rounded-xl text-xs font-semibold text-amber-400 transition-all duration-200 flex items-center justify-center gap-2">
                        {historyVisible[groupKey] ? '🔼 Hide History' : (() => {
                          const shownGroups = new Set();
                          let uniqueRepayments = 0;
                          group.allRepayments.forEach(r => {
                            if (r.groupId) {
                              if (!shownGroups.has(r.groupId)) { shownGroups.add(r.groupId); uniqueRepayments++; }
                            } else { uniqueRepayments++; }
                          });
                          return `📜 Transaction History (${group.individualDebts.length + uniqueRepayments} entries)`;
                        })()}
                      </button>
                        ...
                        <div className="bg-gray-950/50 rounded-xl p-4 mt-2 border border-white/5">
                          {group.individualDebts.map((d, idx) => (
                            <div key={d.firestoreId || idx} className="py-2 border-b border-white/3 last:border-0">
                              ...
                              {d.dueDate && !d.paid && (
                                <>
                                  <div className="text-xs text-red-400 mt-1">⚠ Due: {formatDate(d.dueDate)}</div>
                                  {new Date(d.dueDate) < new Date() && (() => {
                                    const dueDt = new Date(d.dueDate + 'T00:00:00');
                                    const today = new Date();
                                    const diffMonths = Math.max(1, Math.floor((today - dueDt) / (1000 * 60 * 60 * 24 * 30)));
                                    const penaltyAmount = d.returnExpected * (d.penaltyRate / 100) * diffMonths;
                                    return (<div className="text-xs text-red-500 mt-0.5">💸 Penalty: {formatCurrency(penaltyAmount)} ({diffMonths} month{diffMonths > 1 ? 's' : ''} @ {d.penaltyRate}%)</div>);
                                  })()}
                                </>
                              )}
                            </div>
                          ...
                          {/* Group repayments display */}
                          {(() => {
                            const shownGroups = new Set();
                            const groupedRepayments = [];
                            const standaloneRepayments = [];
                            
                            group.allRepayments.forEach(r => {
                              if (r.groupId) {
                                if (!shownGroups.has(r.groupId)) {
                                  shownGroups.add(r.groupId);
                                  groupedRepayments.push({ ...r, amount: r.groupTotal || r.amount, isGrouped: true, id: r.groupId });
                                }
                              } else {
                                standaloneRepayments.push(r);
                              }
                            });
                            
                            const displayRepayments = [...groupedRepayments, ...standaloneRepayments];
                            displayRepayments.sort((a, b) => new Date(b.date) - new Date(a.date));
                            
                            return displayRepayments.map((r) => (
                              <div key={r.id} className="py-2 border-b border-white/3 last:border-0">
                                <div className="flex justify-between items-center mb-1">
                                  <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-bold bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded uppercase tracking-wider">Repay</span>
                                    <span className="text-xs text-slate-500">{formatDate(r.date)}</span>
                                    {r.isGrouped && (
                                      <span className="text-[10px] text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded">
                                        {group.individualDebts.filter(d => d.repayments.some(rp => rp.groupId === r.groupId)).length} debts
                                      </span>
                                    )}
                                  </div>
                                  <button 
                                    onClick={() => {
                                      if (r.isGrouped) {
                                        if (!window.confirm(`Delete this group repayment of ${formatCurrency(r.amount)}?`)) return;
                                        group.individualDebts.forEach(d => {
                                          d.repayments.forEach((rp, i) => {
                                            if (rp.groupId === r.groupId) {
                                              handleDeleteRepayment(d.firestoreId, i, group.displayName);
                                            }
                                          });
                                        });
                                      } else {
                                        handleDeleteRepayment(r.debtId, r.repayIndex, group.displayName);
                                      }
                                    }} 
                                    className="text-[10px] text-red-400 hover:text-red-300 hover:bg-red-500/10 px-2 py-0.5 rounded transition-all" 
                                    title="Delete"
                                  >
                                    Delete🗑
                                  </button>
                                </div>
                                <div className="text-sm font-bold text-emerald-400">{formatCurrency(r.amount)}</div>
                                {r.isGrouped && (
                                  <div className="text-[10px] text-slate-500 mt-0.5">
                                    Applied across {group.individualDebts.filter(d => d.repayments.some(rp => rp.groupId === r.groupId)).length} debts
                                  </div>
                                )}
                              </div>
                            ));
                      ...
                      <>
                        {repayState.active && (
                          <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto">
                            <input type="number" placeholder="Amount" value={repayState.amount} onChange={e => handleRepayAmountChange(groupKey, e.target.value)} className="w-24 px-3 py-2 bg-gray-950/70 border border-white/10 rounded-lg text-slate-100 text-xs outline-none focus:border-amber-500" />
                            <input type="date" value={repayState.date} onChange={e => handleRepayDateChange(groupKey, e.target.value)} className="px-3 py-2 bg-gray-950/70 border border-white/10 rounded-lg text-slate-100 text-xs outline-none focus:border-amber-500" />
                            <button onClick={() => submitRepayment(groupKey)} className="px-3 py-2 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-xs rounded-lg transition-all">✓ Save</button>
                          </div>
                        )}
                        <button onClick={() => toggleRepayInput(groupKey)} className="px-3 py-2 bg-amber-500/15 hover:bg-amber-500/25 text-amber-400 border border-amber-500/30 text-xs font-semibold rounded-lg transition-all whitespace-nowrap">{repayState.active ? 'Cancel' : '💰 Repay'}</button>
                      </>
                    )}
      ...
      {showProfileModal && (
        <>
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100]" onClick={() => setShowProfileModal(false)}></div>
          <div className="fixed inset-0 flex items-center justify-center z-[101] p-4">
            <div className="bg-gray-900 border border-white/10 rounded-3xl p-6 w-full max-w-md shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-amber-400">{editingProfile ? 'Edit Profile' : selectedProfile ? 'Debtor Profile' : 'Create Profile'}</h2>
                <button onClick={() => setShowProfileModal(false)} className="text-slate-400 hover:text-white text-xl transition-colors">✕</button>
              </div>
              {!editingProfile && selectedProfile ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-16 h-16 rounded-full bg-amber-500/20 flex items-center justify-center text-3xl font-bold text-amber-400">{selectedProfile.name[0]?.toUpperCase()}</div>
                    <div><h3 className="text-lg font-bold">{selectedProfile.name}</h3><p className="text-xs text-slate-500">Debtor since {formatDate(selectedProfile.createdAt?.split('T')[0])}</p></div>
                  </div>
                  {selectedProfile.phone && (<div className="flex items-center gap-3 p-3 bg-gray-950/50 rounded-xl"><span className="text-xl">📱</span><div><p className="text-xs text-slate-500">Phone</p><p className="text-sm font-semibold">{selectedProfile.phone}</p></div></div>)}
                  {selectedProfile.email && (<div className="flex items-center gap-3 p-3 bg-gray-950/50 rounded-xl"><span className="text-xl">📧</span><div><p className="text-xs text-slate-500">Email</p><p className="text-sm font-semibold">{selectedProfile.email}</p></div></div>)}
                  {selectedProfile.address && (<div className="flex items-center gap-3 p-3 bg-gray-950/50 rounded-xl"><span className="text-xl">📍</span><div><p className="text-xs text-slate-500">Address</p><p className="text-sm font-semibold">{selectedProfile.address}</p></div></div>)}
                  {selectedProfile.notes && (<div className="p-3 bg-gray-950/50 rounded-xl"><p className="text-xs text-slate-500 mb-1">📝 Notes</p><p className="text-sm">{selectedProfile.notes}</p></div>)}
                  <div className="flex gap-3"><button onClick={() => setShowProfileModal(false)} className="flex-1 py-3 border border-white/10 text-slate-400 rounded-xl font-semibold hover:bg-white/5 transition-all">Close</button><button onClick={() => setEditingProfile(true)} className="flex-1 py-3 bg-amber-500 hover:bg-amber-400 text-gray-900 font-bold rounded-xl transition-all">✏️ Edit</button></div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div><label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Name *</label><input type="text" value={profileForm.name} onChange={e => setProfileForm(prev => ({ ...prev, name: e.target.value }))} className="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-sm outline-none focus:border-amber-500" /></div>
                  <div><label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Phone</label><input type="tel" value={profileForm.phone} onChange={e => setProfileForm(prev => ({ ...prev, phone: e.target.value }))} className="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-sm outline-none focus:border-amber-500" /></div>
                  <div><label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Email</label><input type="email" value={profileForm.email} onChange={e => setProfileForm(prev => ({ ...prev, email: e.target.value }))} className="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-sm outline-none focus:border-amber-500" /></div>
                  <div><label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Address</label><input type="text" value={profileForm.address} onChange={e => setProfileForm(prev => ({ ...prev, address: e.target.value }))} className="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-sm outline-none focus:border-amber-500" /></div>
                  <div><label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Notes</label><textarea rows="3" value={profileForm.notes} onChange={e => setProfileForm(prev => ({ ...prev, notes: e.target.value }))} className="w-full bg-gray-900/60 border border-white/10 rounded-xl px-4 py-3 text-slate-100 text-sm outline-none focus:border-amber-500" /></div>
                  <div className="flex gap-3"><button onClick={() => { setEditingProfile(false); if (!selectedProfile) setShowProfileModal(false); }} className="flex-1 py-3 border border-white/10 text-slate-400 rounded-xl font-semibold hover:bg-white/5 transition-all">{selectedProfile ? 'Cancel Edit' : 'Cancel'}</button><button onClick={handleSaveProfile} className="flex-1 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-gray-900 font-bold rounded-xl hover:from-amber-400 hover:to-amber-500 transition-all">💾 Save</button></div>
                </div>
              )}
            </div>
          </div>
